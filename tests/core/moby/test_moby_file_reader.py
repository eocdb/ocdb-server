...
class MobyFileReaderTest(unittest.TestCase):
    ...
    def test_read(self, fn, debug=False):
         reader = MobyFileReader(fn)
         data_record = reader.read(fn)
         if debug:
            print(data_record)

    def setUp(self, fn):
        self.reader = MobyFileReader(fn)
    ...
    def test_fhds_comments_missing(self):
        moby_file = ['File: 1997072021D.MBY\n',
        ...
        with self.assertRaises(MobyFormatError) as cm:
            self.reader._parse(moby_file)
        self.assertEqual('FhdS comments missing.', f"{cm.exception}")
    ...
    def test_parse_location_in_header_time_info_in_header_and_records(self):
        ...
        dataset = self.reader._parse(moby_file)
