...
class TestMongoDbDriver(unittest.TestCase):
    ...
    def test_insert_two_and_get_by_metadata_field(self):
        ...
        self.assertEqual("archive", result.datasets[0].path)
        self.assertEqual("dataset-4.txt", result.datasets[0].filename)
    ...
    def test_insert_three_and_get_by_metadata_field_and(self):
        ...
        result = self._driver.find_datasets(query)
        self.assertEqual(1, result.total_count)
        self.assertEqual("dataset-5.txt", result.datasets[0].filename)
        self.assertEqual("archive", result.datasets[0].path)
    ...
    def test_insert_three_and_get_by_metadata_field_or(self):
        ...
        self.assertEqual("archive", result.datasets[0].path)
        self.assertEqual("dataset-6.txt", result.datasets[0].filename)
        ...
        self.assertEqual(2, result.total_count)
        self.assertEqual("archive", result.datasets[0].path)
        self.assertEqual("dataset-7.txt", result.datasets[0].filename)
    ...
    def test_insert_and_get_by_path(self):
        ...
        query = DatasetQuery(expr="path:archive")

        result = self._driver.find_datasets(query)
        self.assertEqual(2, result.total_count)
        self.assertEqual("archive", result.datasets[0].path)
        self.assertEqual("dataset-9.txt", result.datasets[0].filename)

    def test_insert_and_get_by_path_single_char_wildcard(self):
        dataset = helpers.new_test_dataset(13)
        dataset.path = "/home/ocdb/store/BIGELOW/BALCH/gnats/archive/chl/chl-s170604w.sub"
        self._driver.add_dataset(dataset)

        dataset = helpers.new_test_dataset(14)
        dataset.path = "/home/ocdb/store/BIGELOW/BALCH/gnats/archive/chl/chl-s170605w.sub"
        self._driver.add_dataset(dataset)

        dataset = helpers.new_test_dataset(15)
        dataset.path = "/home/ocdb/store/BIGELOW/BALCH/gnats/archive/chl/chl-s170804w.sub"
        self._driver.add_dataset(dataset)

        query = DatasetQuery(expr="path:/home/ocdb/store/BIGELOW/BALCH/gnats/archive/chl/chl-s17060?w.sub")

        result = self._driver.find_datasets(query)
        self.assertEqual(2, result.total_count)
        self.assertEqual("/home/ocdb/store/BIGELOW/BALCH/gnats/archive/chl/chl-s170604w.sub",
                         result.datasets[0].path)
        self.assertEqual("/home/ocdb/store/BIGELOW/BALCH/gnats/archive/chl/chl-s170605w.sub",
                         result.datasets[1].path)

    def test_insert_and_get_by_path_multi_char_wildcard(self):
        dataset = helpers.new_test_dataset(13)
        dataset.path = "archive"
        dataset.filename = "Helga.txt"
        self._driver.add_dataset(dataset)

        dataset = helpers.new_test_dataset(14)
        dataset.path = "archive"
        dataset.filename = "Helma.txt"
        self._driver.add_dataset(dataset)

        dataset = helpers.new_test_dataset(15)
        dataset.path = "bla"
        dataset.filename = "Olga.txt"
        self._driver.add_dataset(dataset)

        query = DatasetQuery(expr="path:*ar")

        result = self._driver.find_datasets(query)
        self.assertEqual(2, result.total_count)
        self.assertEqual("Helga.txt", result.datasets[0].filename)
        self.assertEqual("archive", result.datasets[0].path)
        self.assertEqual("archive", result.datasets[1].path)
        self.assertEqual("Helma.txt", result.datasets[1].filename)
    ...
    def test_insert_and_get_by_qc_status(self):
        ...
        self.assertEqual(1, result.total_count)
        self.assertEqual("archive", result.datasets[0].path)
        self.assertEqual("dataset-13.txt", result.datasets[0].filename)
    ...
    def test_get_offset_only(self):
        ...
        self.assertEqual("archive", result.datasets[0].path)
        self.assertEqual("dataset-0.txt", result.datasets[0].filename)
        ...
        self.assertEqual(10, result.total_count)
        self.assertEqual("archive", result.datasets[0].path)
        self.assertEqual("dataset-1.txt", result.datasets[0].filename)
    ...
    def test_get_offset_and_negative_count(self):
        ...
        self.assertEqual("archive", result.datasets[0].path)
        self.assertEqual("dataset-9.txt", result.datasets[9].filename)
        ...
        self.assertEqual("archive", result.datasets[0].path)
        self.assertEqual("dataset-3.txt", result.datasets[0].filename)
    ...
    def test_insert_two_and_get_by_location(self):
        ...
        self.assertEqual("archive", result.datasets[0].path)
        self.assertEqual("dataset-12.txt", result.datasets[0].filename)
    ...
    def test_insert_two_and_get_by_location_many_records_with_geojson_one_result(self):
        ...
        self.assertEqual("archive", result.datasets[0].path)
        self.assertEqual("dataset-15.txt", result.datasets[0].filename)
    ...
    def test_insert_two_and_get_by_location_many_records_with_geojson_two_results(self):
        ...
        self.assertEqual("archive", result.datasets[0].path)
        self.assertEqual("dataset-17.txt", result.datasets[0].filename)

        self.assertEqual("archive", result.datasets[1].path)
        self.assertEqual("dataset-19.txt", result.datasets[1].filename)
    ...
    def test_insert_two_and_get_by_location_and_metadata(self):
        ...
        self.assertEqual("archive", result.datasets[0].path)
        self.assertEqual("dataset-16.txt", result.datasets[0].filename)
    ...
    def test_insert_two_and_get_by_time_interval(self):
        ...
        self.assertEqual(1, result.total_count)
        self.assertEqual("archive", result.datasets[0].path)
        self.assertEqual("dataset-18.txt", result.datasets[0].filename)
        ...
        self.assertEqual("archive", result.datasets[0].path)
        self.assertEqual("dataset-18.txt", result.datasets[1].filename)
    ...
    def test_insert_two_and_get_by_product_group_non_matching(self):
        dataset = helpers.new_test_db_dataset(18)
        dataset.groups = ["a", "b"]
        self._driver.add_dataset(dataset)

        dataset = helpers.new_test_db_dataset(19)
        dataset.groups = ["Chl_a", "b"]
        self._driver.add_dataset(dataset)
    ...
    def test_insert_two_and_get_by_product_group_one_matching(self):
        ...
        self.assertEqual("archive", result.datasets[0].path)
        self.assertEqual("dataset-21.txt", result.datasets[0].filename)

    def test_insert_two_and_get_by_product_group_both_matching(self):
        ...
        self.assertEqual(2, result.total_count)
        self.assertEqual("archive", result.datasets[0].path)
        self.assertEqual("dataset-22.txt", result.datasets[0].filename)
        self.assertEqual("archive", result.datasets[1].path)
        self.assertEqual("dataset-23.txt", result.datasets[1].filename)

    def test_insert_two_and_get_by_product_group_two_groups_both_matching(self):
    ...
    def test_insert_two_and_get_by_product_name_non_matching(self):
        dataset = helpers.new_test_db_dataset(18)
        dataset.attributes = ["a", "b"]
        self._driver.add_dataset(dataset)

        dataset = helpers.new_test_db_dataset(19)
        dataset.attributes = ["c", "d"]
        self._driver.add_dataset(dataset)

        query = DatasetQuery(pname=["e"])
        result = self._driver.find_datasets(query)
        self.assertEqual(0, result.total_count)

    def test_insert_two_and_get_by_product_name_one_matching(self):
        dataset = helpers.new_test_db_dataset(20)
        dataset.attributes = ["a", "b"]
        self._driver.add_dataset(dataset)
        ...
        query = DatasetQuery(pname=["b"])
        result = self._driver.find_datasets(query)
        self.assertEqual(1, result.total_count)

        self.assertEqual("archive", result.datasets[0].path)
        self.assertEqual("dataset-20.txt", result.datasets[0].filename)

    def test_insert_two_and_get_by_product_name_two_matching(self):
        dataset = helpers.new_test_db_dataset(22)
        dataset.attributes = ["a", "b"]
        self._driver.add_dataset(dataset)
    ...
    def test_insert_two_and_get_by_measurement_type(self):
        ...
        result = self._driver.find_datasets(query)
        self.assertEqual(1, result.total_count)
        self.assertEqual("dataset-43.txt", result.datasets[0].filename)
        self.assertEqual("archive", result.datasets[0].path)
    ...
    def test_insert_one_and_update(self):
        ...
        result = self._driver.get_dataset(ds_id)
        self.assertEqual("archive", result.path)
        self.assertEqual("dataset-44.txt", result.filename)
    ...
    def test_shallow_no(self):
        ...
        self.assertEqual(2, result.total_count)
        self.assertEqual("archive", result.datasets[0].path)
        self.assertEqual("dataset-28.txt", result.datasets[0].filename)
        self.assertEqual("archive", result.datasets[1].path)
        self.assertEqual("dataset-29.txt", result.datasets[1].filename)
    ...
    def test_shallow_exclusively(self):
        ...
        self.assertEqual(1, result.total_count)
        self.assertEqual("archive", result.datasets[0].path)
        self.assertEqual("dataset-33.txt", result.datasets[0].filename)
    ...
    def test_to_dataset_ref_without_points(self):
        ...
        self.assertEqual("test.txt", dataset_ref.filename)
    ...
    def test_convert_times_empty(self):
        dic = {'path': 'archive',
               'filename': 'dataset-17.txt',
               'times': []}

        converted_dict = self._driver._convert_times(dic)
    ...
    def test_add_submission_and_get_one_by_userid(self):
        ...
        sf = DbSubmission(submission_id="the_first_test", date=date, user_id='5876123', status=QC_STATUS_SUBMITTED,
                          qc_status="OK", path="walking", files=[], store_user_path='Tom_Helge')
        ...
        self.assertEqual(QC_STATUS_SUBMITTED, result_sub.status)
    ...
    def test_add_submission_with_files_get_one_by_userid(self):
        ...
        sf = DbSubmission(submission_id=submission_id, date=date, user_id='5876123', status=QC_STATUS_SUBMITTED,
                          qc_status="OK",
                          path="/the/cool/path", files=[sf_0, sf_1], store_user_path='Tom_Helge')
    ...
    def test_add_three_submission_and_get_two_by_userid(self):
        sf = DbSubmission(submission_id="the_first_test", date=datetime(2018, 4, 23, 12, 15, 34), user_id='5876123',
                          status=QC_STATUS_SUBMITTED, qc_status="OK", path="/hey/yes/", files=[],
                          store_user_path='Tom_Helge')
        sf_id = self._driver.add_submission(sf)
        self.assertIsNotNone(sf_id)

        sf = DbSubmission(submission_id="the_other_user_thing", date=datetime(2017, 4, 23, 12, 15, 34),
                          user_id='999999',
                          status=QC_STATUS_VALIDATED, qc_status="OK", path="some/where/else", files=[],
                          store_user_path='Tom_Helge')
        sf_id = self._driver.add_submission(sf)
        self.assertIsNotNone(sf_id)

        sf = DbSubmission(submission_id="the_second_one", date=datetime(2011, 1, 22, 12, 15, 34), user_id='5876123',
                          status=QC_STATUS_PUBLISHED, path="whatever", qc_status="OK", files=[],
                          store_user_path='Tom_Helge')
        ...
        self.assertEqual('5876123', result_sub.user_id)
        self.assertEqual(QC_STATUS_PUBLISHED, result_sub.status)
    ...
    def test_add_submission_and_get_file(self):
        ...
        sf = DbSubmission(submission_id=submission_id, date=date, user_id='5876123', status=QC_STATUS_SUBMITTED,
                          qc_status="OK",
                          path="/some/where/", files=[sf_0, sf_1], store_user_path='Tom_Helge')
        ...
        self.assertEqual(QC_STATUS_SUBMITTED, result.status)
    ...
    def test_add_submission_and_get_file_invalid_index(self):
        ...
        sf = DbSubmission(submission_id=submission_id, date=date, user_id='5876123', status=QC_STATUS_SUBMITTED,
                          qc_status="OK",
                          path="p/a/th/", files=[sf_0, sf_1], store_user_path='Tom_Helge')
    ...
    def test_add_submission_and_get_file_no_files(self):
        ...
        submission = DbSubmission(submission_id=submission_id, date=date, user_id='5876123', status="SUBMITTED",
                                  qc_status="OK",
                                  path="/root", files=[], store_user_path='Tom_Helge')
    ...
    def test_update_submission_invalid_id(self):
        submission = DbSubmission(submission_id="dunno_", date=datetime(2018, 3, 22, 11, 14, 33), user_id='5876123',
                                  ...
                                  qc_status="OK",
                                  path="/root", files=[], id_="nasenmann", store_user_path='Tom_Helge')
    ...
    def test_insert_submission_and_delete(self):
        ...
        submission = DbSubmission(submission_id=submission_id, date=datetime(2019, 2, 22, 11, 14, 33),
                                  user_id='5876123',
                                  status=QC_STATUS_SUBMITTED,
                                  qc_status="OK",
                                  path="/root", files=[], store_user_path='Tom_Helge')
    ...
    def test_get_user_not_exists(self):
        result = self._driver.get_user('god')
        self.assertIsNone(result)
